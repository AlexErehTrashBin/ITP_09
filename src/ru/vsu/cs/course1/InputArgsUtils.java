...
public class InputArgsUtils {
	...
	public static void individualFileCheck(String inFile, String outFile){
		...
		int[] array = ArrayUtils.readIntArrayFromFile(inFile);
		if (array == null) {
			System.err.printf("Не удалось прочитать тестовый массив. %n%n");
			System.exit(1);
		}
		System.out.printf("Входной массив: %s %n", ArrayUtils.toString(array));
		ArrayList<Integer> arrayList = TaskUtils.convertArrayToList(array);
		Task.process(arrayList);
		int[] result = TaskUtils.convertListToArray(arrayList);
		try {
			PrintStream psArr = new PrintStream(outFile);
			System.out.println("Ответ: " + ArrayUtils.toString(result));
			psArr.println(ArrayUtils.toString(result));
			psArr.close();
		} catch (Exception e){
